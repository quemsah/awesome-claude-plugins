...
function seeded() {
  ...
  initializeSchema(db)
  for (const key of ['seed_repos_sha256', 'seed_stats_sha256', 'seed_repos_count', 'seed_stats_count']) setSetting(db, key, '1')
  return db
}
...
describe('crawlSchedule', () => {
  ...
  it('refuses to crawl an unseeded database and invalid publication settings', () => {
    const db = new Database(':memory:')
    initializeSchema(db)
    expect(() => crawlSchedule(db, new Date(), 72, true)).toThrow(/unseeded/)
    for (const key of ['seed_repos_sha256', 'seed_stats_sha256', 'seed_repos_count', 'seed_stats_count']) setSetting(db, key, '1')
