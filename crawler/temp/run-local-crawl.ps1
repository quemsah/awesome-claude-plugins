...
$existingDb = Get-ChildItem -LiteralPath $runsRoot -Filter 'catalog.sqlite' -Recurse -File |
  Sort-Object { $_.Directory.Name } -Descending |
  Select-Object -First 1
$baselineDurationMs = $null
if ($existingDb) {
  $dbPath = $existingDb.FullName
  Write-Host "Reusing local database: $dbPath"
  $baselineRaw = & node $progressPath $dbPath
  if ($LASTEXITCODE -ne 0) { throw "Could not inspect existing database: $dbPath" }
  $baseline = ConvertFrom-Json -InputObject ($baselineRaw -join '')
  if ($baseline.status -eq 'running') {
    throw "A crawl is still running in $dbPath. Wait for it to finish before starting another crawl."
  }
  ...
  Write-Host "No existing local database found; creating: $dbPath"
}
...
function Write-ProgressSnapshot {
  ...
  $script:progressSamples.Add([pscustomobject]@{ at = $now; pending = [double]$progress.pending })
  while ($script:progressSamples.Count -gt 31) { $script:progressSamples.RemoveAt(0) }
  $pendingRate = 0
  if ($script:progressSamples.Count -gt 1) {
    $windowStart = $script:progressSamples[0]
    $windowMinutes = ($now - $windowStart.at).TotalMinutes
    if ($windowMinutes -ge 10) {
      $pendingRate = ($windowStart.pending - $progress.pending) / $windowMinutes
    }
  }
  if ($progress.status -ne 'running') {
    $eta = if ($progress.status -eq 'completed') { 'complete' } else { 'n/a' }
  } elseif ($progress.phase -eq 'enrichment' -and $pendingRate -gt 0) {
    $etaMinutes = $progress.pending / $pendingRate
    $eta = "~$([math]::Floor($etaMinutes / 60))h $([math]::Floor($etaMinutes % 60))m ($([math]::Floor($windowMinutes))-min net queue rate)"
...
$script:progressSamples = [System.Collections.Generic.List[object]]::new()
