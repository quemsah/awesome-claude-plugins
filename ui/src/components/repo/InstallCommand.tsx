  ...
  return (
    <div className="px-6 pb-4">
      <div className="flex items-center gap-2 rounded-md border bg-muted/50 p-3">
        ...
        {installCommand && !isVerified && <span className="shrink-0 text-xs text-muted-foreground">unverified</span>}
        <button
          aria-label={isCopied ? 'Installation command copied' : 'Copy installation command'}
          className={`shrink-0 rounded-md p-1 transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${isCopied ? 'bg-green-500/20 text-green-600' : isVerified ? 'hover:bg-muted' : 'opacity-50 cursor-not-allowed'}`}
